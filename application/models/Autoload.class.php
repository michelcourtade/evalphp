...
class Autoload
{
	...
	public function load($classname)
	{
		...
		if (strpos(strtolower($classname), 'smarty_') === 0 || strpos(strtolower($classname), 'twig_') === 0)
			return;
	...
	public function generateIndex()
	{
		$classes = array_merge(
			...
			//$this->getClassesFromDir('override/models/'),
		...
		if ((file_exists($filename) && !is_writable($filename)) || !is_writable(dirname($filename)))
			throw new Exception($filename.' is not writable, please give write permissions (chmod 666) on this file.');
		else
		{
			...
			if ($file[0] != '.')
			{
				...
				else if (substr($file, -4) == '.php')
				{
					$content = file_get_contents($this->root_dir.$path.$file);
			 		$pattern = '#\W((abstract\s+)?class|interface)\s+(?P<classname>'.basename($file, '.class.php').'(Core)?)'
